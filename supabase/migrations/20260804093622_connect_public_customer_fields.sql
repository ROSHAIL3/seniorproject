...
begin
  ...
  if submission_id is null
    ...
    or jsonb_typeof(coalesce(target_customer_field_values, '{}'::jsonb))
      <> 'object'
    or pg_column_size(coalesce(target_customer_field_values, '{}'::jsonb))
      > 32768
  ...
  insert into public.public_booking_attempts (
    ...
    organization.id, $13, normalized_customer_phone
  );
  ...
  begin
    ...
    -- Reject an existing customer's overlapping request before changing their
    -- profile. The customer advisory lock makes this check concurrency-safe.
  ...
  exception
    ...
    when others then
      if sqlerrm like '%SERVICE_FIELD_%'
        or sqlerrm like '%CUSTOMER_FIELD_%' then
        return jsonb_build_object(
...
comment on function booking_private.create_public_booking(
  ...
  'Creates an idempotent public booking request with validated customer/service fields, shared locks, and database overlap protection.';


create or replace function booking_private.get_public_booking_page_v2(
  booking_slug text
)
returns jsonb
language plpgsql
...
declare
  organization public.organizations :=
    booking_private.public_booking_organization(booking_slug);
  page_data jsonb := booking_private.get_public_booking_page(booking_slug);
begin
      ...
      select jsonb_agg(
        jsonb_build_object(
          ...
          'options', coalesce((
            ...
            from public.customer_field_options option
...
set search_path = ''
as $$
  select booking_private.get_public_booking_page_v2(booking_slug);
$$;
...
revoke all on function booking_private.get_public_booking_page_v2(text)
  from public;
...
grant execute on function booking_private.get_public_booking_page_v2(text)
...
comment on function booking_private.get_public_booking_page_v2(text) is
  'Returns the public booking page plus active reduced customer field definitions.';
comment on function public.create_public_booking(
  text, uuid, text, text, timestamptz, text, text, text, text,
  jsonb, jsonb, uuid, text
) is
  'Server-only public booking request entry point with customer custom fields.';
