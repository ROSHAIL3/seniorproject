...
export default function PhoneInput({ id, name, value, defaultValue = "", placeholder = "Phone number", onChange, onBlur, error = false, disabled = false, autoComplete = "tel", ariaLabel = "Phone number", countries = phoneCountries, defaultCountry = "BH", className = "" }: PhoneInputProps) {
  ...
  return <div ref={wrapperRef} className={`relative ${className}`}>
    <div className={`flex h-10 w-full rounded-lg border shadow-theme-xs focus-within:ring-3 ${stateClasses}`}>
      <button type="button" disabled={disabled} aria-expanded={open} aria-controls={listId} aria-label="Choose phone country" onClick={() => setOpen((current) => !current)} className="flex min-w-[68px] items-center justify-center gap-1.5 rounded-l-lg border-r border-gray-200 px-2.5 text-[13px] disabled:cursor-not-allowed dark:border-gray-700"><span aria-hidden>{flag(selected.code)}</span><ChevronDownIcon className="size-3.5 text-gray-500 dark:text-gray-400" /></button>
      <span className="inline-flex shrink-0 items-center px-2.5 text-[13px] font-medium text-gray-700 dark:text-gray-300" aria-label={`${selected.name} calling code`}>{selected.callingCode}</span>
      <input id={id} name={name} type="tel" inputMode="tel" value={displayedNumber} disabled={disabled} placeholder={placeholder} autoComplete={autoComplete} aria-label={ariaLabel} aria-invalid={error || undefined} onBlur={onBlur} onChange={(event) => { const next = digitsOnly(event.target.value); if (value === undefined) setLocalNumber(next); emit(selected, next); }} className="min-w-0 flex-1 rounded-r-lg bg-transparent px-2.5 py-2 text-[13px] outline-none placeholder:text-gray-400 disabled:cursor-not-allowed dark:placeholder:text-white/30" />
    </div>
    {open && !disabled && <div id={listId} className="absolute z-50 mt-2 w-full min-w-72 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-theme-lg dark:border-gray-700 dark:bg-gray-dark"><div className="border-b border-gray-100 p-2 dark:border-gray-800"><div className="relative"><SearchIcon className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-gray-400" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search country or code" className="h-9 w-full rounded-lg border border-gray-300 bg-white pl-9 pr-3 text-[13px] text-gray-800 shadow-theme-xs outline-none transition-colors hover:border-gray-400 focus:border-brand-400 focus:ring-3 focus:ring-brand-500/15 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:hover:border-gray-600 dark:focus:border-brand-700" /></div></div><div className="max-h-64 overflow-y-auto p-1.5">{filtered.map((country) => <button key={country.code} type="button" onClick={() => choose(country)} className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-[13px] hover:bg-gray-100 dark:hover:bg-white/5 ${country.code === selected.code ? "bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400" : "text-gray-700 dark:text-gray-300"}`}><span className="text-base" aria-hidden>{flag(country.code)}</span><span className="min-w-0 flex-1 truncate">{country.name} <span className="text-xs text-gray-400">({country.code})</span></span><span className="font-medium">{country.callingCode}</span></button>)}{!filtered.length && <p className="px-3 py-4 text-center text-[13px] text-gray-500">No countries found.</p>}</div></div>}
  </div>;
}
