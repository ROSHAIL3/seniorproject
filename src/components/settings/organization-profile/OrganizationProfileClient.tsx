...
import type { Organization, OrganizationDetails, OrganizationProfileOptions, OrganizationStorageUsage, PublicBookingSettings } from "@/types/organization";
...
export default function OrganizationProfileClient({ initialOrganization, options }: { initialOrganization: Organization; options: OrganizationProfileOptions }) {
  ...
  const [storage, setStorage] = useState<OrganizationStorageUsage>(initialOrganization.storage);
  ...
  const handleLogo = async (file?: File) => {
    if (!file) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type)) { setErrors({ logo: "Choose a PNG, JPG or WebP image." }); return; }
  ...
  const confirmDelete = async () => {
    try {
      ...
      setDeleted(true); setDeleteOpen(false); setConfirmationName("");
      router.replace("/signin?error=organization-deleted");
      router.refresh();
    } catch (error) { showError(error); }
  };
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <section aria-labelledby="danger-zone-heading"><ComponentCard title="Danger Zone" desc="Restricted organization actions." className="border-error-200 dark:border-error-500/30" headerClassName="px-4 py-4 sm:px-6" bodyClassName="p-4 sm:p-6" contentClassName="space-y-3"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm font-medium text-gray-800 dark:text-white/90">Delete Organization</p><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Disable this organization and sign out. Data is retained for possible administrative recovery.</p></div><Button size="sm" variant="outline" startIcon={<TrashBinIcon />} onClick={() => setDeleteOpen(true)} disabled={deleted} className="!text-error-600 !ring-error-300 !transition-all duration-200 enabled:hover:!bg-error-50 enabled:hover:!text-error-700 enabled:hover:!ring-error-400 enabled:hover:shadow-[0_0_0_3px_rgba(240,68,56,0.14),0_0_20px_rgba(240,68,56,0.24)] focus-visible:shadow-[0_0_0_3px_rgba(240,68,56,0.16),0_0_18px_rgba(240,68,56,0.22)] dark:!bg-gray-800 dark:!text-error-400 dark:!ring-error-500/50 dark:enabled:hover:!bg-error-500/10 dark:enabled:hover:!text-error-300 dark:enabled:hover:!ring-error-400 dark:enabled:hover:shadow-[0_0_0_3px_rgba(240,68,56,0.2),0_0_22px_rgba(240,68,56,0.34)]">{deleted ? "Organization Deleted" : "Delete Organization"}</Button></div></ComponentCard></section>

      <Modal isOpen={deleteOpen} onClose={() => { setDeleteOpen(false); setConfirmationName(""); setErrors({}); }} className="m-4 max-w-lg p-6 sm:p-8"><h3 className="pr-12 text-xl font-semibold text-gray-800 dark:text-white/90">Delete Organization</h3><p className="mt-3 text-sm text-gray-500 dark:text-gray-400">This disables access and signs you out. The stored data is retained for possible administrative recovery.</p><div className="mt-5"><Label>Type <strong>{details.name}</strong> to confirm</Label><Input value={confirmationName} onChange={(event) => setConfirmationName(event.target.value)} error={!!errors.confirmationName} autoComplete="off" />{errors.confirmationName && <FieldError>{errors.confirmationName}</FieldError>}</div><div className="mt-6 flex justify-end gap-3"><Button size="sm" variant="outline" onClick={() => { setDeleteOpen(false); setConfirmationName(""); }}>Cancel</Button><Button size="sm" onClick={confirmDelete} disabled={confirmationName !== details.name} startIcon={<TrashBinIcon />} className="bg-error-500 hover:bg-error-600 disabled:bg-error-300">Delete Organization</Button></div></Modal>
    </div>
  );
}

function StorageSection({ storage }: { storage: OrganizationStorageUsage }) {
  const percent = useMemo(() => Math.min(100, storage.limitBytes ? (storage.usedBytes / storage.limitBytes) * 100 : 0), [storage]);
  return <section aria-labelledby="storage-usage-heading"><ComponentCard title="Logo Storage Usage" desc="Free-plan-safe usage for the organization logo." headerClassName="px-4 py-4 sm:px-6" bodyClassName="p-4 sm:p-6" contentClassName="space-y-3"><div className="flex justify-between gap-4 text-sm"><span className="font-medium text-gray-800 dark:text-white/90">{formatStorageSize(storage.usedBytes)} used</span><span className="text-gray-500 dark:text-gray-400">{formatStorageSize(storage.limitBytes)} maximum logo size</span></div><div className="h-2.5 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-800"><div className="h-full rounded-full bg-brand-500 transition-[width]" style={{ width: `${percent}%` }} /></div><div className="flex justify-between gap-4 text-xs text-gray-500 dark:text-gray-400"><span>{storage.uploadedFileCount} current logo</span><span>{percent.toFixed(1)}% of logo limit</span></div><p className="text-xs text-gray-400">This is the per-logo limit, not a per-organization storage allowance. The Supabase Free project has one shared storage quota.</p></ComponentCard></section>;
}
