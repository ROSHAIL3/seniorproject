...
export default function ServiceDetailsPageClient({ serviceId, initialService, initialCategories, initialTeamMembers, initialBranches, initialFields }: { serviceId: string; initialService: Service | null; initialCategories: ServiceCategory[]; initialTeamMembers: TeamMember[]; initialBranches: Branch[]; initialFields: ServiceBookingFieldDefinition[] }) {
  ...
  return <ServiceDetailsClient initialService={data.service} categories={data.categories} teamMembers={data.teamMembers} branches={initialBranches} initialFields={data.fields} />;
}
