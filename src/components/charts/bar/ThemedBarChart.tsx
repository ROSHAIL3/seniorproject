...
export default function ThemedBarChart({ data, seriesName, height = 260, minWidth = 640, loading = false, error, emptyMessage = "No chart data available.", valueFormatter = (value) => String(value), lineSeriesName }: ThemedBarChartProps) {
  ...
  const options = useMemo<ApexOptions>(() => ({
    ...
    chart: { fontFamily: "var(--font-space-grotesk), sans-serif", type: lineSeriesName ? "line" : "bar", height, toolbar: { show: false }, animations: { enabled: true } },
    theme: { mode: theme },
    ...
    xaxis: { categories: data.map((item) => item.label), axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { colors: chartTextColor, fontSize: "12px" } } },
    yaxis: { labels: { formatter: valueFormatter, style: { colors: [chartTextColor], fontSize: "12px" } } },
    ...
    fill: { opacity: 1 },
    legend: { show: Boolean(lineSeriesName), position: "top", horizontalAlign: "left", labels: { colors: chartTextColor } },
    tooltip: { theme, x: { show: true }, y: { formatter: valueFormatter, title: { formatter: () => `${seriesName}: ` } } },
  }), [chartTextColor, data, height, lineSeriesName, seriesName, theme, valueFormatter]);
