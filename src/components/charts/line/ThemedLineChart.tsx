...
export default function ThemedLineChart({ categories, series, height = 310, minWidth = 720, loading = false, error, emptyMessage = "No chart data available.", valueFormatter = (value) => String(value) }: { categories: string[]; series: LineChartSeries[]; height?: number; minWidth?: number; loading?: boolean; error?: string; emptyMessage?: string; valueFormatter?: (value: number) => string }) {
  ...
  const options = useMemo<ApexOptions>(() => ({
    ...
    chart: { background: "transparent", foreColor: "var(--color-gray-500)", fontFamily: "Outfit, sans-serif", height, type: "area", toolbar: { show: false }, animations: { enabled: true } },
    legend: { show: series.length > 1, position: "top", horizontalAlign: "left", labels: { colors: "var(--color-gray-500)" } },
    ...
    xaxis: { type: "category", categories, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false }, labels: { style: { colors: "var(--color-gray-500)", fontSize: "12px" } } },
    yaxis: { labels: { formatter: valueFormatter, style: { fontSize: "12px", colors: ["var(--color-gray-500)"] } } },
  }), [categories, height, series, theme, valueFormatter]);
